...
class ReservationForm(forms.ModelForm):
    ...
    def save(self, commit=True):
        ...
        if reservation.reservation_time in time_mapping:
            start_time_str, ent_time_str = (
                time_mapping[reservation.reservation_time].split(' - ')
            )
            reservation.start_time = timezone.datetime.strptime(
                start_time_str, '%H:%M'
            ).time()
            reservation.end_time = timezone.datetime.strptime(
                ent_time_str, '%H:%M'
            ).time()
